...
class GatedFusionLayer(nn.Module):
    def __init__(self, d, d_inner, encode_length, n_position=200):
        ...
        self.wave = nn.Sequential(
            nn.Linear(d, int(1.5 * d), bias=False),
            nn.Linear(int(1.5 * d), 2 * d, bias=False),
            nn.Linear(2 * d, int(1.5 * d), bias=False),
            nn.Linear(int(1.5 * d), d, bias=False)
        )
        self.hidden1 = nn.Linear(d, d_inner, bias=False)
        ...
        self.linear = nn.Linear(d, d_inner, bias=False)
        self.decode_length = n_position - encode_length
    ...
    def forward(self, x_A, x_B, x_L):
        ...
        #x_L = self.wave(x_L)
        #h_A = self.tanh_f(self.hidden1(x_A))
        #h_B = self.tanh_f(self.hidden1(x_B))
        #h_L = self.tanh_f(self.hidden1(x_L))
        h_A = self.relu(self.hidden1(x_A))
        h_B = self.relu(self.hidden2(x_B))
        h_L = self.relu(self.hidden3(x_L))
        #h_A = self.tanh_f(h_A)
        #h_B = self.tanh_f(h_B)
        #h_L = self.tanh_f(h_L)
        x = torch.cat((x_A, x_B, x_L), dim=-1)
        z1 = self.relu(self.gated1(x)) * h_A
        z2 = self.relu(self.gated1(x)) * h_B
        z3 = self.relu(self.gated1(x)) * h_L
        #z1 = self.sigmoid_f(self.gated1(x)) * h_A
        #z2 = self.sigmoid_f(self.gated2(x)) * h_B
        #z3 = self.sigmoid_f(self.gated3(x)) * h_L
