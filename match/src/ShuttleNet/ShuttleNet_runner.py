...
def shotGen_trainer(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config, device="cpu"):
    ...
    for epoch in tqdm(range(config['epochs']), desc='Epoch: '):
        ...
        for loader_idx, item in enumerate(data_loader):
            ...
            loss_shot = criterion['entropy'](output_shot_logits, target_shot.long())
...
def multi_shotGentrainer(data_loader_area, data_loader_shoot, encoder, decoder, criterion, encoder_optimizer,decoder_optimizer, config, device):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    record_loss = {
        'total': [],
        'shot': [],
    ...
    total_loss, total_shot_loss, total_area_loss = 10, 10, 10
    i=0
    ep_to_eva = 50
    for epoch in tqdm(range(config['epochs']), desc='Epoch: '):
        encoder.train(), decoder.train()
        if i%ep_to_eva==0:
            if total_area_loss > 1.2:
                ep_to_eva = 100
                data_loader = data_loader_area
            elif total_shot_loss > 1.2:
                ep_to_eva = 50
                data_loader = data_loader_shoot
            else:
                data_loader = data_loader_shoot
        ...
        for loader_idx, item in enumerate(data_loader):
            batch_input_shot, batch_input_x, batch_input_y, batch_input_player = item[0].to(device), item[1].to(device), \
                                                                                item[2].to(device), item[3].to(device)
            batch_target_shot, batch_target_x, batch_target_y, batch_target_player = item[4].to(device), item[5].to(device), \
                                                                                 item[6].to(device), item[7].to(device)
            seq_len, seq_sets = item[8].to(device), item[9].to(device)
