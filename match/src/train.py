...
from ShuttleNet.ShuttleNet_runner import shotGen_trainer , multi_shotGentrainer
...
if __name__ == "__main__":
    ...
    #config1, train_dataloader1, val_dataloader1, test_dataloader1, train_matches1, val_matches1, test_matches1 = prepare_dataset(config, "train.csv")
    #config2, train_dataloader2, val_dataloader2, test_dataloader2, train_matches2, val_matches2, test_matches2 = prepare_dataset(config, "augment_databyarea")
    config1, train_dataloader_area, val_dataloader, test_dataloader, train_matches, val_matches, test_matches = prepare_dataset(config, "augment_databyarea.csv")
    config, train_dataloader_shoot, val_dataloader, test_dataloader, train_matches, val_matches, test_matches = prepare_dataset(config, "train.csv")
    ...
    # read model
    config["'uniques_type'"] = ['short service', 'net shot', 'lob', 'clear', 'drop', 'push/rush', 'smash', 'defensive shot', 'drive', 'long service']
    ...
    encoder.load_state_dict(torch.load('model/best_1/encoder'))
    decoder.load_state_dict(torch.load('model/best_1/decoder'))
    ...
    record_train_loss = multi_shotGentrainer(data_loader_area=train_dataloader_area, data_loader_shoot= train_dataloader_shoot, encoder=encoder, decoder=decoder,
