...
class BadmintonDataset(Dataset):
    def __init__(self, matches, config):
        ...
        group = matches[['rally_id', 'ball_round', 'type', 'landing_x', 'landing_y', 'player', 'set', 'player_location_area', 'opponent_location_area']].groupby('rally_id').apply(lambda r: (r['ball_round'].values, r['type'].values, r['landing_x'].values, r['landing_y'].values, r['player'].values, r['set'].values, r['player_location_area'].values, r['opponent_location_area'].values))

        self.sequences, self.rally_ids = {}, []
        for i, rally_id in enumerate(group.index):
            ball_round, shot_type, landing_x, landing_y, player, sets, p_area, o_area = group[rally_id]
            self.sequences[rally_id] = (ball_round, shot_type, landing_x, landing_y, player, sets, p_area, o_area)
    ...
    def __getitem__(self, index):
        ...
        ball_round, shot_type, landing_x, landing_y, player, sets, p_area, o_area = self.sequences[rally_id]
        ...
        pad_input_p_area = np.full(self.max_ball_round, fill_value=PAD, dtype=int)
        pad_input_o_area = np.full(self.max_ball_round, fill_value=PAD, dtype=int)
        ...
        pad_output_p_area = np.full(self.max_ball_round, fill_value=PAD, dtype=int)
        pad_output_o_area = np.full(self.max_ball_round, fill_value=PAD, dtype=int)
        ...
        if len(ball_round) > self.max_ball_round:
            ...
            pad_input_p_area[:] = p_area[0:-1:1][:rally_len]
            pad_input_o_area[:] = o_area[0:-1:1][:rally_len]
            ...
            pad_output_p_area[:] = p_area[1::1][:rally_len]
            pad_output_o_area[:] = o_area[1::1][:rally_len]
            ...
            pad_input_p_area[:rally_len] = p_area[0:-1:1]
            pad_input_o_area[:rally_len] = o_area[0:-1:1]
            ...
            pad_output_p_area[:rally_len] = p_area[1::1]
            pad_output_o_area[:rally_len] = o_area[1::1]

        return (pad_input_shot, pad_input_x, pad_input_y, pad_input_player, pad_input_p_area, pad_input_o_area,
                pad_output_shot, pad_output_x, pad_output_y, pad_output_player, pad_output_p_area, pad_output_o_area,
                rally_len, sets[0])


def prepare_dataset(config):
    train_matches = pd.read_csv(f"{config['data_folder']}train/mirror_10_match_10.csv")
