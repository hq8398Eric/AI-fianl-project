...
if __name__ == "__main__":
    ...
    print(f"{current_model_path}prediction.csv")
    ...
    for rally_id in tqdm(testing_rallies):
        # read data
        selected_matches = val_matches.loc[(val_matches['rally_id'] == rally_id)][['rally_id', 'type', 'landing_x', 'landing_y', 'player_location_area', 'opponent_location_area', 'player', 'rally_length']].reset_index(drop=True)
        ...
        given_seq = {
            ...
            'given_p_area': torch.tensor(selected_matches['player_location_area'].values).to(device),
            'given_o_area': torch.tensor(selected_matches['opponent_location_area'].values).to(device),
